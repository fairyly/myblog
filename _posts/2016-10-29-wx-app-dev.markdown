...
页面图片src用数据绑定,显示图片的时候，pages中注册页面加载顺序也得变下才会显示；

canvas绘制图片的时候模拟器不会绘制显示出来，真机可以；包括上传的图片也可以绘制，取得是src,即res.tempFilePaths[0]

input模拟器与真机差异：模拟器样式可以有边框，文字居中；真机没有边框，文字没有居中，一直左对齐

view设置fixed，里面一个input，bottom设置0，输入法键盘唤出总会遮挡input一部分；


post 的时候把Content-Type 这个去掉 就行了 
